...
------------_--------------_--------------_--------------_--------------_-------
'''
# Take a predefined quaternion array, process it into a combined (RA,dec,roll)
# The input quaternion is defined such that q = [q_vec;q_scal], where the quaternion
#   vector elements q_vec are listed first, and the quaternion scalar element q_scal last

# Example of (10,30,0)
#inputQuaternion = np.array([ 0.02255757, -0.25783416,  0.08418598,  0.96225019])

quatObject      = Quat(inputQuaternion)

# Convert to angles
ra_from_quat = np.float(quatObject.ra)
dec_from_quat= np.float(quatObject.dec)
roll_from_quat=np.float(quatObject.roll)

# provide a label, that sets the directory in which outputs will be stored
inputs['camera']['label'] = 'demonstration_without_roll'

# what is the commanded central ra and dec of the field?
inputs['camera']['ra'] = ra_from_quat
inputs['camera']['dec'] = dec_from_quat
inputs['camera']['roll'] = roll_from_quat
...
inputs['catalog']['skykw']['faintlimit'] = 7.0
...
inputs['observation']['cadencestodo'] = {2:3}
